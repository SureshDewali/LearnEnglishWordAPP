  ...
  const renderView = () => {
    ...
    switch (state.view) {
      ...
      case 'learning': return <LearningView 
        ...
        onSpeak={speak}
        onNext={() => state.currentWordIndex < state.dailyWords.length - 1 ? setState(p => ({ ...p, currentWordIndex: p.currentWordIndex + 1 })) : setState(p => ({ ...p, view: 'completion' }))}
      ...
      case 'quiz-score': return <QuizScoreView score={state.finalScore} total={state.dailyWords.length} onContinue={() => setState(p => ({ ...p, view: 'quiz-final' }))} onHome={goHome} />;
...
const LandingView: React.FC<{ onSelect: (l: Language) => void }> = ({ onSelect }) => (
  <div className="flex-1 p-6 flex flex-col items-center justify-between h-full pt-32 pb-12">
    ...
    {/* Refined Slogan - Only on Landing Page */}
    ...
    <div className="flex flex-col gap-4 w-full max-w-[320px]">
      ...
      <button onClick={() => onSelect(Language.NEPALI)} className="glass-premium p-6 flex items-center gap-5 border-white/20 group transition-all active:scale-95 shadow-2xl">
        ...
        <div className="text-left">
          <h3 className="text-base font-black text-white uppercase tracking-tight text-readable">English-Nepali</h3>
        </div>
      </button>
      ...
      <button onClick={() => onSelect(Language.HINDI)} className="glass-premium p-6 flex items-center gap-5 border-white/20 group transition-all active:scale-95 shadow-2xl">
        <span className="text-4xl group-active:scale-110 transition-transform drop-shadow-md">🇮🇳</span>
        <div className="text-left">
          <h3 className="text-base font-black text-white uppercase tracking-tight text-readable">English-Hindi</h3>
        </div>
      </button>
    </div>
  </div>
    ...
    <div className="flex-1 p-6 flex flex-col items-center justify-center h-full pt-24">
      ...
      <div className="w-full max-w-[320px]">
        ...
        <div className="glass-premium p-8 text-center relative overflow-hidden border-white/20 mb-6">
          ...
          >
            {lock.locked ? `Wait: ${lock.remaining}` : `▶️ Start Drills`}
          </button>
        </div>
        ...
        <div className="glass-premium overflow-hidden">
          <button 
            ...
            className="w-full p-5 flex items-center justify-between font-black text-white text-[11px] uppercase tracking-widest active:bg-white/5 text-readable"
          >
            ...
            <div className="p-4 pt-0 border-t border-white/10 bg-white/[0.03] max-h-[160px] overflow-y-auto custom-scroll">
              {completedDays.length > 0 ? (
                <div className="grid grid-cols-3 gap-2 py-3">
                  {completedDays.map(d => (
                    <button key={d} onClick={() => onSelectDay(d, true)} className="bg-white/10 hover:bg-emerald-500/20 text-white font-black text-[10px] py-4 rounded-xl border border-white/10 active:scale-95 transition-colors text-readable">
                      Day {d}
                    </button>
                  ))}
                </div>
...
const LearningView: React.FC<{ words: Word[], index: number, day: number, onSpeak: (t: string) => void, onNext: () => void, onPrev: () => void, onHome: () => void }> = ({ words, index, day, onSpeak, onNext, onPrev, onHome }) => {
  ...
  return (
    <div className="h-full w-full flex flex-col items-center justify-between overflow-hidden p-5 pt-32 pb-12">
      ...
      <div className="w-full max-w-[360px] flex flex-col h-full justify-between">
        {/* Compact Progress */}
        <div className="flex justify-between items-center mb-4 px-2">
          ...
          <div className="flex gap-1.5">
            {words.map((_, i) => (<div key={i} className={`h-1.5 rounded-full transition-all duration-300 ${i === index ? 'w-6 bg-emerald-500' : 'w-1.5 bg-white/30'}`} />))}
          </div>
        </div>

        {/* HEIGHT CONSTRAINED CARD - NO-SCROLL COMPLIANCE */}
        <div className="glass-premium p-6 border-white/20 shadow-3xl flex-1 flex flex-col justify-center max-h-[55%] min-h-0">
          ...
          <div className="space-y-4 overflow-hidden">
            <div className="flex flex-col items-start gap-1">
              <div className="text-xl font-black text-emerald-400 uppercase tracking-tight leading-tight text-readable">{word.meaning}</div>
              
              {word.synonym && (
                <div className="inline-flex items-center gap-2 bg-white/10 px-2 py-0.5 rounded-lg border border-white/10">
            ...
            <div className="bg-black/35 p-4 rounded-2xl border border-white/10 shadow-inner overflow-y-auto custom-scroll max-h-[160px]">
              <p className="text-[13px] text-white italic font-medium leading-relaxed mb-2 text-readable">"{word.english_sentence}"</p>
              <p className="text-[10px] text-emerald-100 font-bold uppercase tracking-widest leading-tight text-readable">{word.native_sentence}</p>
            </div>
          </div>
        </div>
...
const CompletionView: React.FC<{ onStart: () => void, onHome: () => void }> = ({ onStart, onHome }) => (
  <div className="flex-1 flex flex-col items-center justify-center p-10 h-full text-center pt-24">
    <GlobalHeader onHome={onHome} />
    <div className="text-7xl mb-8 drop-shadow-xl">🎓</div>
    <h2 className="text-3xl font-black mb-2 text-white uppercase italic tracking-tighter text-readable">Session Done</h2>
    <p className="text-slate-100 text-[11px] mb-12 uppercase tracking-[0.4em] font-bold text-readable">Ready for Knowledge Check</p>
    <button onClick={onStart} className="btn-skeuo-elite btn-gold-elite w-full max-w-[280px] py-7 text-[12px] font-black uppercase tracking-widest shadow-2xl active:scale-95 text-readable">Begin Drills</button>
  </div>
);
    ...
    <div className="flex-1 p-6 flex flex-col items-center justify-center h-full pt-24">
      ...
      <div className="w-full max-w-[340px]">
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-black text-white leading-snug text-readable">
             Translate: <br/><span className="text-3xl text-emerald-300 italic">"{word.word}"</span>
          </h2>
        </div>
        <div className="grid grid-cols-1 gap-4">
          {options.map((opt, i) => {
            let style = "glass-premium assessment-option text-[15px] font-black uppercase transition-all shadow-xl active:scale-95 text-readable";
            if (locked) {
              if (opt === word.meaning) style = "bg-emerald-600/90 border-transparent assessment-option text-[16px] font-black uppercase shadow-2xl scale-105 z-10 text-readable";
              else if (opt === selected) style = "bg-rose-600/90 border-transparent opacity-60 assessment-option text-[16px] font-black uppercase text-readable";
...
const QuizScoreView: React.FC<{ score: number, total: number, onContinue: () => void, onHome: () => void }> = ({ score, total, onContinue, onHome }) => (
  <div className="flex-1 flex flex-col items-center justify-center p-12 h-full text-center pt-24">
    <GlobalHeader onHome={onHome} />
    <div className="w-44 h-44 glass-premium flex flex-col items-center justify-center border-2 border-white/20 mb-10 shadow-3xl">
    ...
    <button onClick={onContinue} className="btn-skeuo-elite btn-gold-elite w-full max-w-[280px] py-7 text-[12px] font-black uppercase tracking-widest active:scale-95 shadow-2xl text-readable">Final Phase</button>
  </div>
);
...
const QuizFinalView: React.FC<{ words: Word[], lang: Language, onComplete: (score: number) => void, onHome: () => void }> = ({ words, lang, onComplete, onHome }) => {
  ...
  const questions = useMemo<FinalQuestion[]>(() => {
    ...
    return shuffledPool.map((w, idx) => {
      ...
      switch(type) {
        case 'meaning-match':
          prompt = `Precise ${targetLang} term for "${w.word}":`;
          correct = w.meaning;
          options = [w.meaning, ...words.filter(x => x.id !== w.id).map(x => x.meaning).slice(0, 3)];
          break;
        case 'fill-blank':
          prompt = `Missing term: "${w.english_sentence.replace(new RegExp(`\\b${w.word}\\b`, 'gi'), '_____')}"`;
          correct = w.word;
          options = [w.word, ...words.filter(x => x.id !== w.id).map(x => x.word).slice(0, 3)];
          break;
        case 'sentence-completion':
          prompt = `In "${w.native_sentence}", which English root matches?`;
          correct = w.word;
          options = [w.word, ...words.filter(x => x.id !== w.id).map(x => x.word).slice(0, 3)];
          break;
        case 'synonym-logic':
          prompt = `Closest lexicon peer for "${w.word}":`;
          correct = w.synonym;
          options = [w.synonym, ...words.filter(x => x.id !== w.id).map(x => x.synonym).slice(0, 3)];
          break;
        case 'translation-verify':
          prompt = `Verify correct contextual usage of "${w.word}":`;
          correct = w.english_sentence;
          options = [w.english_sentence, ...words.filter(x => x.id !== w.id).map(x => x.english_sentence).slice(0, 3)];
          break;
      }
      ...
      <div className="w-full max-w-[360px]">
        <div className="text-center mb-6">
          ...
          <div className="glass-premium p-5 bg-black/45 border-white/20 min-h-[120px] flex items-center justify-center mb-8 shadow-xl">
            <p className="text-[16px] font-black text-white italic leading-relaxed text-readable">{q.prompt}</p>
          </div>
        </div>
        <div className="grid grid-cols-1 gap-4">
          {q.options.map((opt, i) => {
            let style = "glass-premium assessment-option text-[14px] font-black uppercase transition-all shadow-xl active:scale-95 text-readable";
            if (locked) {
              if (opt === q.correct) style = "bg-emerald-600/80 border-transparent assessment-option text-[15px] font-black shadow-2xl scale-105 z-10 text-readable";
              else if (opt === selected) style = "bg-rose-600/80 border-transparent opacity-60 assessment-option text-[15px] font-black text-readable";
              else style += " opacity-20 pointer-events-none";
            }
...
const SummaryView: React.FC<{ day: number, score: number, total: number, onRevise: () => void, onNextDay: () => void, onHome: () => void }> = ({ day, score, total, onRevise, onNextDay, onHome }) => {
  ...
  return (
    <div className="flex-1 p-10 flex flex-col items-center justify-center h-full text-center pt-24">
      ...
      <h2 className="text-4xl font-black mb-3 text-white uppercase italic tracking-tighter text-readable">
        Day {day} Mastery
      </h2>
      <p className="text-emerald-400 text-[11px] font-black tracking-[0.5em] mb-16 uppercase text-readable">{score}/{total} DRILLS PASSED</p>
